...
$queryEscola = "SELECT idEscola, nomeEscola FROM Escola WHERE idEscola = 7";
...
if ($idAluno) {
    $queryQuestoes = "SELECT * FROM Questoes_mat WHERE idAluno = ?";
...
$queryAlunos = "SELECT idAluno, nomeAluno FROM Aluno WHERE idEscola = 7";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['atualizar'])) {
    ...
    $sqlUpdate = "UPDATE Questoes_mat SET 
...
<body>
    ...
    <form method="POST" action="">
        ...
        <?php if (!empty($respostas)): ?>
            <fieldset>
                ...
                <?php for ($i = 1; $i <= 20; $i++): ?>
                    ...
                    <select name="resposta<?= $i ?>" id="resposta<?= $i ?>" required>
                        ...
                        <option value="NF" <?= $respostas["resposta$i"] == "NF" ? "selected" : "" ?>>NF</option>
                        <option value="X" <?= $respostas["resposta$i"] == "X" ? "selected" : "" ?>>X</option>
                    </select>
                    <br>
                <?php endfor; ?>
            </fieldset>
            ...
            <a href="inserirResp_MAT.php" class="linkacesso-link">Voltar</a>
        <?php else: ?>
