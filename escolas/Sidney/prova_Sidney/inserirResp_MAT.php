...
// Recuperar Alunos que ainda não têm respostas na tabela Questoes_lp
$queryAlunos = "SELECT idAluno, nomeAluno FROM Aluno 
                WHERE idEscola = 7 AND idAluno NOT IN (SELECT DISTINCT idAluno FROM Questoes_mat)";
$resultAlunos = $conn->query($queryAlunos);

$queryEscola = "SELECT nomeEscola FROM Escola WHERE idEscola = 7";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enviar'])) {
    ...
    /* Busca idDisciplina associado ao idAluno, filtrando por idDisciplina entre 6 e 10,
    pois no banco de dados na tabela Disciplina os IDs de 6 a 10 representam as classes
    do 1º ao 5º ano somente na disciplina de Matemática*/
    ...
    $queryDisciplinas = "SELECT DISTINCT ad.idDisciplina, e.idEscola
                         ...
                         WHERE ad.idAluno = ? AND ad.idDisciplina BETWEEN 6 AND 10";
    ...
    if (!$disciplinasAssociadas) {
        echo "<script>alert('O aluno não está vinculado a disciplinas com ID entre 6 e 10.')</script>";
        exit();
    }
    ...
    $sqlInsert = "INSERT INTO Questoes_mat 
...
<body>
    ...
    <form method="POST" action="">
        ...
        <fieldset>
            <legend>
                ATENÇÃO: Respostas somente letras de A até D e verificar a quantidade de questões:
                1º ano: LP: 16, MAT: 13; 2º ano: LP: 15, MAT: 14; 3º ano: LP: 14, MAT: 15; 4º ano: LP: 15, MAT: 17; 5º ano: LP: 16, MAT: 17. Coloque "NF" nas alternativas não feitas pelo aluno e "X" nas últimas que não existem. 
            </legend>
        ...
        <a href="editarResp_MAT.php" class="linkacesso-link">Editar resposta</a>
        <a href="inicio.php" class="linkacesso-link">Escolher disciplina</a>
